"""
Batch wrapper for my_deepocsort_es_v0.py: traverses a root hierarchy with
Annotation/ and Data/ and generates results in Results/DeepOCSort/ for all
...
from my_deepocsort import Config, run as run_tracker
...
# BATCH PROCESSING
# ==============================
def process_scene(root: Path, scene_ann_dir: Path, model_path: Path) -> None:
    ...
    out_scene_dir = root / "Results/DeepOCSort" / scene_label_name
...
def main():
    p = argparse.ArgumentParser(
        description="Processes all scenarios/cameras from a root directory containing Annotation/ and Data/ folders, and generates tracking results in Results/DeepOCSort/"
    )
    p.add_argument("--root", required=True, type=Path, help="Root directory containing Annotation/ and Data/ folders")
    p.add_argument("--model_path", required=True, type=Path, help="Path to the ReID weights (.pt)")
    ...
    for scene_ann_dir in scenes:
        process_scene(root, scene_ann_dir, args.model_path)
