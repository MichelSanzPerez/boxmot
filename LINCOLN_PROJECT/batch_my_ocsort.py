"""
Batch wrapper for my_ocsort.py: traverses a root hierarchy with
Annotation/ and Data/ and generates results in Results/OCSort/ for all
...
from my_ocsort import Config, run as run_tracker
...
def process_scene(root: Path, scene_ann_dir: Path) -> None:
    ...
    out_scene_dir = root / "Results/OCSort" / scene_label_name
...
def main():
    p = argparse.ArgumentParser(
        description="Processes all scenarios/cameras from a root directory containing Annotation/ and Data/ folders, and generates tracking results in Results/OCSort/"
    )
