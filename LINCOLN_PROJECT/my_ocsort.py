...
# TRACKER FACTORY
# ==============================
def build_tracker() -> OcSort:
    """Instantiate OCSort aligned with the effective repo/YAML behavior."""
    return OcSort(
        per_class=False,        # OCSort no soporta seguimiento por clase, así que forzamos a False
        min_conf=0.1,           # Umbral de confianza mínimo para considerar detecciones (ajustable según necesidades)
        det_thresh=0.6,         # Umbral de confianza para usar detecciones (puede ser más alto que min_conf para filtrar detecciones débiles)
        max_age=30,             # Máximo número de frames para mantener un track perdido antes de eliminarlo
        min_hits=3,             # Mínimo número de detecciones para iniciar un track (puede ser 1 para iniciar con una sola detección)
        asso_threshold=0.3,     # Umbral de asociación (IoU mínimo para asociar detecciones a tracks)
        delta_t=3,              # Número de frames para estimar la velocidad
        asso_func="iou",        # Función de asociación (IoU es común, pero OCSort también soporta otras como "giou")
        inertia=0.1,            # Peso de inercia para suavizar el movimiento de los tracks (0.0 = sin inercia, 1.0 = solo inercia)
        use_byte=False,         # OCSort no requiere ByteTrack, así que lo desactivamos
        Q_xy_scaling=0.01,      # Escalado para la matriz de covarianza en la actualización de Kalman (ajustable según la dinámica del movimiento)
        Q_s_scaling=0.0001,     # Escalado para la matriz de covarianza del tamaño en la actualización de Kalman (ajustable según la variabilidad del tamaño de los objetos)
    )
...
# ==============================
def main():
    p = argparse.ArgumentParser(description="OCSort wrapper for MOT tracking from JSON detections")
