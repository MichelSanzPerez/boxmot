"""
Batch wrapper for my_strongsort_es_v0.py: traverses a root hierarchy with
Annotation/ and Data/ and generates results in Results/StrongSort/ for all
...
from my_strongsort import Config, run as run_tracker
...
def process_scene(root: Path, scene_ann_dir: Path, args) -> None:
    ...
    out_scene_dir = root / "Results/StrongSort" / scene_label_name
...
def main():
    p = argparse.ArgumentParser(
        description="Processes all scenarios/cameras from a root directory containing Annotation/ and Data/ folders, and generates tracking results in Results/StrongSort/"
    )
